...
class Mover:
    ...
    def __init__(self):
        ...
        self.publisher2 = rospy.Publisher('/thorvald_002/teleop_joy/cmd_vel', Twist, queue_size=1)
        
        rospy.Subscriber("/thorvald_001/front_scan", LaserScan, self.callback)
        rospy.Subscriber("/thorvald_002/front_scan", LaserScan, self.callback2)
    ...
    def callback(self, data):
        ...
        rospy.loginfo(
            "The closest point in laser frame coords from robot1 is at\n%s"
            % pose.pose.position
            )
        ...
        rospy.loginfo(
            "The closest point in robot1 coords is at\n%s"
    ...
    def callback2(self, data):
        ...
        rospy.loginfo(
            "The closest point in laser frame coords from robot2 is at\n%s"
            % pose.pose.position
            )

        # now lets actually transform this pose into a robot 
        # "base_link" frame.
        transformed_pose2 = self.listener.transformPose("thorvald_002/base_link",pose)
        rospy.loginfo(
            "The closest point in robot2 coords is at\n%s"
            %transformed_pose2.pose.position
            )
